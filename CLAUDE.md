...
**第四種：粒度都對，但取錯了桶。基線的小時一律由「視窗涵蓋的時間」決定，
不是 `window_end`**（2026-08-06 修）。基線的桶是 `toStartOfHour(create_time)`，
而規則的視窗是 `[window_end - window_minutes, window_end)` —— **往回看的**。
`engine._resolve_threshold()` 一度直接用 `window_end.hour`，於是**每小時必有一個
tick 的 window_end 落在整點**，那時 60 分鐘視窗的內容剛好是前一個完整整點小時、
卻查到**下一個**小時的分位數。

實測（正式環境 2026-08-06，20 筆 R04 事件裡 19 筆）：傍晚流量逐小時陡降，
`Api2/GivePoint` 在 21:00~22:00 的正常量 2,117（前七天同時段 1,768~2,097）
拿 h22 的門檻 2,000 來比就命中；它自己 h21 的門檻是 5,083。整批事件的
metric/threshold 比值全部落在 1.00~1.14、同一組 endpoint 每天重複 ——
**「每天固定發、而且都只超過一點點」就是這種偏差的指紋**。
`Api2/MixTrans` 在 h21 只要 **0.58 倍**於自己的中位數就告警（設計上 2.88 倍）。

**反方向是漏抓，而且那半邊沒有任何告警會提醒人。** 掃過 `api_endpoint_60m`
全部 182 個動態門檻真正生效的格子：44 格偏低（誤報）、39 格偏高 ——
`Api2/GetProfile` 在 h07 要 9.22 倍才叫（應該 3.04 倍）。
跨午夜的視窗還會連 `day_class` 一起取錯（週一 00:00 的 tick 看的是**週日**的
23 點，母體該是 weekend）。

唯一真相是 `baseline.window_point(window_end, window_minutes)` = **視窗中點**。
不用視窗起點：起點在 window_end 不落整點時會反過來錯（22:40 的 tick 視窗
`[21:40, 22:40)` 有 40 分鐘在 h22，起點會給 h21）。中點對 60 分鐘以內的視窗
永遠落在涵蓋分鐘數最多的那一小時。

**實際受影響的規則比「有 baseline_key 的規則」少很多，因為只有三個 metric
家族真的有逐小時的列**：`api_endpoint_60m:`（R04、R11）與 `backend_route_60m:`
（R14）是 60 分鐘視窗、原本 100% 錯一小時；`boss_login_success_10m`（R06）
只在 window_end 落整點的那個 tick 錯。其餘（R03 的 `api_src_ep_60m`、
R15 的 `backend_ip_60m`、R13 的 `brand_store_60m`、R10A/B、R01、R09）
在 calibrate 裡**只寫全域的 `(-1,'all')` 一列**（母體分布刻意不分小時，見下一段），
`baseline.get()` 的回退鏈本來就落在同一列，取哪個小時都一樣 —— 那些規則沒有被這個
bug 影響過。**改動門檻邏輯前先問「這個 key 有逐小時的列嗎」**，答案決定了要不要回測。

`replay` 的實測（2026-08-05 週三正常日，24 個整點 tick）：
R04 **5 → 0** 次命中（消失的正是 EVT-0070／0073／0077／0078／0079 那五筆）、
R14 14 → 14（21:00 視窗少一件、07:00 視窗多一件，那件是原本被抬高的門檻漏抓的）、
R11／R06 兩邊都是 0。**修正沒有製造新的噪音。**
`Explorer` 快速範本的 `_win()` 也回這個點（第三個元素**刻意不是視窗右界**，
免得又有人拿它的 `.hour`）—— 那裡的症狀不是告警而是畫面上的 median 與倍數，
而使用者正是拿那個數字判斷「這個量正不正常」。
`tests/test_baseline_window_hour.py` 用逐分鐘的獨立 oracle 守這一整組，
並對所有帶 `baseline_key` 的規則斷言「取樣點必須落在自己的視窗內」。
**反過來也守**：把它改成「一律減一小時」會在 22:45 的 tick 上失敗。

一律用 **bucket 的起點或視窗的中點**去查基線，不要用「現在」——
`trends.py`／`entity_history.py`／`routes.py` 的 zero-fill 迴圈傳的是 bucket 起點
（正確）；`health.py` 傳 `now` 也是正確的，因為那裡呈現的是「當下這個 10 分鐘
桶的參考值」而不是某個往回看的視窗。
...
`routes.UNJUDGED` 同時是 `GET /api/events` 的 `judgement` 篩選值、`unjudged` 那格
頁籤的成員值、與前端下拉的選項，但 `judge_event` 只接受 `JUDGEMENTS` ——
存得進去卻篩不出來的判定值是這裡最容易出現的漂移，所以提交端與篩選端共用同一組
常數。前端的下拉選項一律來自回應的 `judgements` / `unjudged_label` /
`judgement_tabs[].judgements`，**不自己列一份**（差一個字就是一個永遠篩不到
東西的選項，而畫面完全正常）。

`judgement` 是封閉集合，**打錯一律 400**：靜靜接受的話 `judgement=誤報x` 回 0 筆，
而畫面上的已套用條件寫著「判定 = 誤報x」，讀起來像「這段時間沒有誤報」。
`severity` / `source` / `status` / `tab` 同理（清單的條件現在寫在網址裡、使用者
改得到，「值不存在」與「沒有事件」必須分得開）。同理 `unjudged=true` 與具體判定
同時給是 400 而不是空清單。`unjudged` 保留是為了不破既有測試的契約，
新的呼叫端一律用 `judgement` 或 `tab`。

## 批次判定（`POST /api/events/judge`）

清單頁可勾選多筆一次判定。與單筆端點**共用 `_write_judgement()` 一條寫入路徑**
（兩份 UPDATE 遲早有一份漏掉 `owner` 或把 `judgement_note` 存成非 JSON，
而症狀是「詳細頁讀不回剛剛打的字」，不會有任何錯誤），差異只在呼叫端算好的
`detail`：

**單筆是完整取代，批次是「留空不蓋」。** 單筆表單三個欄位都顯示現值並一起送出，
清空某一欄就是要清空它；批次送的是「這一批共同的說法」，而選取裡可能有別人
已經寫過證據的事件 —— 一律取代的話那些字會靜靜消失，畫面上只會顯示
「已判定 N 筆」。所以批次只套用**有填**的欄位（用鍵的有無表達，不是空字串），
回應帶 `kept`，而那句話要出現在**輸入欄旁邊**，不是只寫在送出後的回應裡。
要清空某一欄請進單筆頁面。`tests/test_batch_judge.py` 兩個方向都守。

**`evt_nos` 有一個查不到就整批 404、一列都不寫。** 清單頁送的就是它剛剛查到的
編號，查不到代表呼叫端送錯了。允許部分成功的話畫面會停在「12 筆裡成功 9 筆」
而使用者無從知道是哪 3 筆。

**稽核逐筆一列，不是一批一列。** target 帶「批次 N 筆」標記。一批一列的話
`/api/audit` 用 evt_no 搜尋會搜不到其中 N-1 筆，而「每一筆判定都查得到」正是
`web/pages/audit-mode.js` 對稽查人員的承諾。

**選取範圍只有表格上那幾列，且改動任何條件就清空。** 前者是因為清單被
`LIMIT 300` 截斷過（「符合條件的全部」會讓人按下去時看不到自己改了哪些，
截斷時操作條要明說還有幾筆不在範圍內）；後者掛在 `commit()` 這個唯一漏斗上
—— 留著的話會對畫面上已經看不到的事件下判定（勾 20 筆 P0 → 改篩選成 P3 →
送出，那 20 筆照樣被判掉而畫面一列都沒有）。**選取刻意不進網址**：網址是拿來
分享畫面的，而「我勾了哪 30 筆」是進行到一半的操作，不是畫面狀態。

**操作條是 `position:fixed` 不是 `sticky`。** `.content` 才是捲動容器，而事件頁
根 div 是 `height:100%` —— sticky 的黏著範圍只有約一個畫面，捲到第 200 列時
操作條會消失，而使用者手上還握著 30 筆選取。`--nav-w` 是側邊選單寬度的唯一真相
（`.nav` 與 `.batchbar` 共用）。

**判定按鈕的選項來自回應的 `judgements`**，前端不列第二份（同判定下拉的理由）。

## 判定頁籤與「網址即狀態」（異常事件清單）

清單頁的組織方式是**分流**而不是篩選：`routes.JUDGEMENT_TABS` 五格
（待判定／已確認攻擊／保持觀察／已排除／全部），進站預設落在待判定。
頁籤只換判定，其餘六個條件跨頁籤留著。

**成員清單只在後端，前端從 `judgement_tabs` 讀。** 前端若有第二份，日後新增第六個
判定值時那個判定的事件會從所有頁籤一起消失，而畫面完全正常
（`test_judgement_tabs_cover_every_judgement` 斷言五格的聯集等於全部判定值）。
`judgement` 因此改為**可重複**（「已排除」要一次帶三個值），另有 `tab=<key>` 簡寫
—— 它存在是因為第一次查詢在拿到 `judgement_tabs` 之前就要送出，沒有它前端就得
寫死成員清單。兩者同時給是 400（不定義誰蓋誰）。

**`by_judgement` 已刪除，取而代之的是 `judgement_tabs[].count`。** 舊的那個鍵是
「套用**全部**篩選之後」算的，於是點進任一格時其餘每一格必然是 0；而頁籤數字的
語意必須是「同樣的條件下，**別格**還有幾筆」（走一趟不含判定條件的
`COUNT(*) GROUP BY judgement`）—— 那是使用者在一格看到 0 筆時唯一的線索，
空狀態就靠它說出「已確認攻擊還有 3 筆」。同一個鍵兩種範圍是這個專案一再出事的
形狀，所以不沿用舊鍵而是直接換名。

**`total` 是真實筆數，不是 `len(events)`。** 頁籤數字是真實計數，若 `total` 仍被
`LIMIT 300` 截斷，同一個畫面上會出現「待判定 512」配「共 300 筆事件」。現在
`total` / `by_severity` / `by_status` 全部走 `_facets()` 的獨立 `COUNT(*)`，
另回 `shown` 與 `truncated`，畫面明說「表格顯示前 300 筆」。

**清單的狀態全部活在網址裡**（`web/pages/events-view.js` 的 `parse` / `stringify` /
`toParams`），所以點進事件再返回、F5、把網址貼給同事都是同一個畫面。四條硬性規則：

- **頁籤走 query 不走路徑段**：`#/events/attack` 會撞 `#/events/EVT-0001`
  （Slack 告警的深連結），同 `#/rules/R06` 那個順序坑。
- **`applyHash` 要先切掉 `?` 再 split `/`**，否則 `events?tab=attack` 整段被當成
  `head`，`TITLES[head]` 查不到就靜靜留在原本那一頁。
- **同頁內的條件變更走 `history.replaceState`**（`syncHash({replace:true})`）。
  每動一個下拉都寫一筆 history 的話，上一頁按鈕會變成逐格倒退篩選歷史、
  退不回原本那一頁。`replaceState` 只傳 `'#/...'`（相對解析），不傳路徑。
- **`applyHash` 落在 events 且 query 與現值不同時要 `eventsKey++`**。`page` 沒變時
  `v-if` 不會重新掛載，畫面會與網址靜靜不一致；而從詳細頁返回時 query 相同、
  不重建。

App 只把那串 query 當**不透明字串**（`eventsQuery`），不認識任何參數名 ——
認識了就會有第二份字彙。它取代了原本的 `eventsFilter` slot，順帶消滅一個既有
bug：`goto()` 只在有帶 filter 時寫它、從來不清空，所以從總覽點過一次「前往判定」
之後那個條件會一直復活。

刻意**不用 `keep-alive`**：網址已經帶著全部狀態，兩個機制會靜靜不一致
（手動改網址時 `page` 沒變、元件不重掛，keep-alive 裡的舊狀態與網址不同）。
代價是捲動位置不留，可接受 —— 返回時本來就要重查（剛判定完的那筆必須從「待判定」
消失，列數本來就會變）。

**跨頁連結要連時間窗一起帶**，否則數字對不上：總覽的 P0–P3 卡片是「固定近 24 小時」
（→ `tab=all&severity=P2&hours=24`，實測不帶會是卡片 25 對上清單 47），
「待判定」區塊是不限時間（→ `hours=2160`，清單查得到的最大值）。
卡片是**不分判定**的筆數，所以落點只能是「全部」那一格 —— 這也正是那一格存在的
理由（少了它就是「卡片寫 3、點進去只有 1」）。
...
**前端讀一個新的後端欄位時，欄位不存在必須降級成「舊行為」，不可以當成 0 或空。**
前端是 `no-store`、重新整理就生效，而 Python 要重啟（`restart_server.ps1` 沒有
`--reload`）—— 所以**「前端新、後端舊」是每次改動的必經中間狀態**，不是異常。
實測：Explorer 的 `hasTrend` 改讀新欄位 `total` 並寫成 `total ?? 0`，在還沒重啟的
後端上讓**每一個查詢都顯示「此時間範圍沒有資料」**，看起來像資料庫掛了或篩選壞了，
完全看不出是少一個欄位。正確寫法是 `total == null ? <舊判斷> : total > 0`。
...
- **時間序列一律由後端零填到「使用者指定的區間」，沒命中的桶補 0**
  （`trends.request_trend` 與 `explorer.trend`，2026-08 補後者）。時間軸是 category、
  等距，所以少一個桶不是「缺一格」而是**時間軸被壓縮**：實測 Explorer 指定
  08-01 00:00 ~ 08-05 08:00 查一個只在 08-04 活動的來源，104 小時只回 9 個點 ——
  左端 83 小時的 0 不存在（圖從 08-04 11:00 開始，看不出那段是 0），而 13:00 的
  下一點直接是 17:00，14–16 點的空白在圖上變成一條往上爬的線。
  三個成對的約束：**① 左界用 `timewin.align_bucket()`**（`align_tick` 在 1d 分桶會
  差一格，cursor 的 key 全部查不到、整張圖靜靜變成 0）；**② 右界不可超過
  `effective_now()`** —— 查「今天」時 end 是 23:59:59，填到 23:00 是一段「還沒發生」
  的假 0，與「這段時間沒有活動」在畫面上一模一樣（截短了就回 `window_note` 說出來）；
  **③ 零填後 `rows` 永遠非空，「有沒有命中」只能問 `total`** —— `api/routes.py` 的
  `empty_reason` 與前端的 `hasTrend` 都改讀 total，否則「0 筆」的自我解釋會從趨勢圖
  消失而畫面上是一整排 0。另外桶數從此只由 (區間 ÷ 分桶) 決定，不再由資料多寡決定
  （稀疏對象原本 3 列、之後 25 萬列），所以 `explorer.MAX_TREND_BUCKETS` 明確擋掉
  並要求改分桶。`tests/test_explorer_trend_window.py` 守這一整組。
