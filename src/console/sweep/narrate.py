...
只有 `report.build()` 的結構化輸出：對象、數字、探針名稱、訊號標籤、限制段落。

**其中的「對象」是原始帳號與原始來源 IP。** 這一段在 2026-08 的呈現政策變更前
寫的是「沒有原始 IP、帳號」，那已經不成立 —— `sweep_findings.entity` 現在存的
就是原值（見 `store/db.py` 的註解與 `core/masking.py` 的模組說明），
而報告的 findings 是原封不動送出去的。宣稱一個不存在的保護比什麼都不說更糟。

**沒有送出的是**：token 值（探針層就是 `token_fp` 指紋）、params／headers 原文、
消費者手機與 Email —— 那些從來沒有進到報告裡，所以不是「記得不要送」而是
「沒有可送的」。

要改成「送出前把對象換成代號」是一個獨立的決定（會讓草稿失去可追查性，
因為讀的人得自己把代號對回清單）。在做那個決定之前，這段說明與前端
（`web/pages/sweep.js` 的 AI 研判區塊）必須誠實寫出現況。
