...
# ── 端到端：SQL、pandas NA 正規化、明細呈現三層的判定必須一致 ──────────────
#
# 上面那幾則是單元層級（掃原始碼、直接測 `result_of()` 的三種值）。這兩則走
# 真的 ClickHouse，證明**整條路徑**同意同一個定義 —— `where_clause()` 的
# `only_error` 條件、`query()` 回來的 `pd.NA` 正規化、`_mask_detail_row()` 的
# 「結果」欄，三者任何一處對 `has_error` 的解讀不同，這裡就會失敗。
#
# 區間是寫死的歷史時段，這是刻意的：`2026-08-06 00:00 ~ 06:00` 實測 13,621 筆
# 裡有 24 筆錯誤。**不用「今天」或「最近 N 小時」** —— 這個 repo 已經出現過一則
# 因此每天壞一小時的時間相依測試（見 tests/test_explorer_trend_window.py 的
# test_future_tail_is_not_drawn_as_zero_and_says_so）。歷史 log 不會被覆寫。

_E2E_WINDOW = {"start": "2026-08-06 00:00:00", "end": "2026-08-06 06:00:00"}


def _e2e_detail(**overrides) -> dict:
    f = explorer.ExplorerFilter(source="api", limit=50, **_E2E_WINDOW, **overrides)
    return explorer.detail(f)


def test_only_error_detail_rows_are_all_errors():
    """`only_error=True` 的每一列都必須是「錯誤」。

    `has_error = 1` 的年代這個查詢在 SQL 端就拋 code 386，所以「回得出列」
    本身就是一個回歸守門；而「每一列都是錯誤」才擋得住恆真的條件。
    """
    d = _e2e_detail(only_error=True)
    assert d["total"] > 0, (
        f"{_E2E_WINDOW} 應該有錯誤請求（實測 24 筆）—— 回 0 筆代表 only_error 的"
...
def test_unfiltered_detail_has_both_outcomes():
    ...
    這是上一則的反面，而且是必要的：只斷言「篩選後全是錯誤」的話，一個把
    **每一列**都判成「錯誤」的實作（例如 `pd.NA is not None` 為真那種寫法）
    也會通過。
    """
    results = {r["result"] for r in _e2e_detail()["rows"]}
